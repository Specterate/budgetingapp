...
with tab3:
    ...
    if selectbox_selection is not None:
        with st.form("edit_category", clear_on_submit=True, border=True):
            ...
            monthly_expenses_update = st.number_input("Monthly Expenses", placeholder=st.session_state.edited_dataframe['monthly'].values, key="monthly_expenses_update")
            yearly_expenses_update = st.number_input("Yearly Expenses", placeholder=st.session_state.edited_dataframe['yearly'].values, key="yearly_expenses_update")
